 ...
 */
final class Version20200215162622 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, name, password, email, roles FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL COLLATE BINARY, name VARCHAR(255) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, roles CLOB DEFAULT \'ROLE_COMMENTATOR\' NOT NULL --(DC2Type:simple_array)
        , password_change_date INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, username, name, password, email, roles) SELECT id, username, name, password, email, roles FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, name, password, email, roles FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, roles CLOB DEFAULT \'ROLE_COMMENTATOR\' NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, username, name, password, email, roles) SELECT id, username, name, password, email, roles FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
